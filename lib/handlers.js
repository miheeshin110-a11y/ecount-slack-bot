...
async function buildInventoryText(resolved, products) {
  ...
  for (const code of prodCodes.slice(0, isAll ? 30 : 10)) {
    ...
    // 창고가 2곳 이상이면 헷갈리지 않게 창고별 상세를 같이 보여줌 (단일 품목 조회일 때만)
    if (!isAll && p.whs.length > 1) lines.push(...p.whs);
  }
...
async function resolveOrder(parsed, customers, products) {
  ...
  if (!parsed.custQuery) {
    ...
    } else messages.push(`거래처 \`${parsed.custQuery}\`를 찾지 못했어요. 정확한 거래처명으로 다시 말씀해주시거나, 신규 거래처라면 사업자등록번호(거래처코드)를 직접 입력해주세요.`);
  }
