...
// 드롭다운(static_select) 메뉴 블록 생성 - 거래처처럼 수량 정보가 필요없는 경우
...
// 품목 후보 각각의 재고수량을 미리 조회해서 드롭다운 라벨에 표시 (선택 전에 수량을 알 수 있게)
async function buildProductCandidateBlocksWithQty(candidates) {
  const withQty = await Promise.all(
    candidates.map(async (c) => {
      try {
        const rows = await getInventory({ prodCd: c.code });
  ...
  const options = withQty.map((c) => {
    const spec = c.spec ? ` ${c.spec}` : "";
    const qtyText = c.qty === null ? "" : ` - 재고 ${fmtQty(c.qty)}`;
    const label = `${c.name}${spec}${qtyText}`.slice(0, 75);
    return {
      text: { type: "plain_text", text: label, emoji: true },
      value: c.name.slice(0, 75),
    };
