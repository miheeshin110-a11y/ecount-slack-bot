...
// 스레드 안의 사람 메시지들을 배열로 반환 (첫 메시지=원 요청, 마지막 메시지=최근 답장 구분용)
async function getThreadUserMessages(channel, thread_ts) {
  const botId = await getBotUserId();
  const replies = await slack.conversations.replies({ channel, ts: thread_ts, limit: 50 });
...
module.exports = { slack, getBotUserId, isBotThread, getThreadUserText, getThreadUserMessages };
